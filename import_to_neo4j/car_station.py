...
# print(x)
stations = list(db['train_staion_detail_final'].find({}))
...
query_create  = ['CREATE (',"",":CarStop { name:'","","',lat : ","",", lng:","",", code:'","","'})"]
...
#1 :node, 3 : tên , 5 :lat , 7 :lng
with driver.session() as graphDB_Session:
    for station in stations:
        query_create[1] = station['MaGa'].replace(" ","").replace("-","_")
        query_create[3] = station['Ten ga']
        print(station['Ten ga'])
        query_create[5] = str(station['Info'][0]['geometry']['location']['lat']) 
        query_create[7] = str(station['Info'][0]['geometry']['location']['lng'])
        query_create[9] = station['MaGa'].replace(" ","").replace("-","_")
